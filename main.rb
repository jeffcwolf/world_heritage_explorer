...
loop do
  ...
  puts "(1) Type '1' to search by site name. [Type '1_list' to print a list of ALL site names (#{@xml_names.uniq.size} total)]"
  puts "(2) Type '2' to search by country name. [Type '2_list' to print a list of ALL country names (#{@xml_states.uniq.size} total)]"
  puts "(3) Type '3' to search by region name. [Type '3_list' to print a list of ALL region names (#{@xml_regions.uniq.size} total)]"
  puts "(4) Type '4' to search by site category. [Type '4_list' to print a list of ALL category names (#{@xml_categories.uniq.size} total)"
  ...
  when '1_list'
    puts @xml_names.uniq.sort
  when '2_list'
    puts @xml_states.uniq.sort
  when '3_list'
    puts @xml_regions.uniq.sort
  when '4_list'
    puts @xml_categories.uniq.sort
